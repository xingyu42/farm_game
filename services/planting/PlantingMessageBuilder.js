/**
 * @fileoverview 种植消息构建工具 - 统一种植系统响应消息格式
 *
 * Input:
 * - (无外部依赖,纯工具类)
 *
 * Output:
 * - PlantingMessageBuilder (default) - 消息构建工具类,提供:
 *   - buildErrorMessage: 构建错误消息
 *   - buildInfoMessage: 构建信息消息
 *   - buildPlantingMessage: 构建种植成功响应
 *   - buildHarvestMessage: 构建收获成功响应
 *   - buildCareMessage: 构建护理成功响应
 *   - buildStatusMessage: 构建状态查询响应
 *
 * Pos: 工具类层,统一种植相关的消息构建逻辑,规范化响应格式
 *
 * 消息格式:
 * - 所有消息包含 success 字段(boolean)
 * - 成功消息包含 data 字段(对象)
 * - 失败消息包含 message 字段(字符串)
 * - 使用统一的图标集(🎉 ❌ ℹ️ ⏰ 🧪)
 */
