...
class StatisticsService {
  ...
  async resetStatistics(userId, statsToReset = []) {
    try {
      ...
      await this.playerDataService.updateComplexField(userId, 'statistics', resetStats.statistics);
  ...
  _calculateStealSuccessRate(stats) {
    const totalAttempts = stats.totalStolenBy + stats.totalStolenFrom;
    if (totalAttempts === 0) return 0;
    
    return Math.round((stats.totalStolenBy / totalAttempts) * 100);
  }
