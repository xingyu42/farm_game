...
class SignInService {
  ...
  async resetConsecutiveDays(userId) {
    try {
      ...
      await this.playerDataService.updateComplexField(userId, 'signIn', playerData.signIn);
      await this.playerDataService.updateSimpleField(userId, 'lastUpdated', playerData.lastUpdated);
