  ...
  async enhanceLand(userId, landId) {
    try {
      return await this.redis.withLock(userId, async () => {
        ...
        logger.info(`[LandService] 玩家 ${userId} 土地 ${landId} 强化成功: 等级 ${currentLevel} -> ${nextLevel}`);
      ...
      // 处理锁相关错误
      if (error.message.includes('无法获取锁') || error.message.includes('操作繁忙')) {
        return { success: false, message: '系统繁忙，请稍后再试。' };
      }

      return { success: false, message: '土地强化失败，请稍后再试。' };
    }
  }
}
